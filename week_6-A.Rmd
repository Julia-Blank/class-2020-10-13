...
beads <- c(rep("red", 500), rep("blue", 490), rep("yellow", 1010))

urn <- tibble(ID = 1:2000,
              color = sample(beads))
...
tibble(replicate = 1:10000) %>%
  ...
  mutate(red_wins = total_red > total_blue) %>% 
  summarize(red_prob = sum(red_wins)/n())
```
...
tibble(numb_red_urn = 400) %>%
  mutate(numb_red_paddle = rbinom(size = 25, n = 1, prob = numb_red_urn/1000))
...
tibble(numb_red_urn = seq(from = 100, to = 700, by = 100)) %>%
  mutate(numb_red_paddle = map_int(numb_red_urn, ~ rbinom(size = 25, n = 1, prob = ./1000))) 
...
```{r}
tibble(numb_red_urn = seq(from = 100, to = 700, by = 100)) %>%
  mutate(numb_red_paddle = map(numb_red_urn, ~ rbinom(size = 25, n = 10, prob = ./1000))) %>% 
